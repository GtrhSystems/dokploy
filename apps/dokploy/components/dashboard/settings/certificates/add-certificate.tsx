...
export const AddCertificate = () => {
	...
	const onSubmit = async (data: AddCertificate) => {
		await mutateAsync({
			...
			serverId: data.serverId === "globalpivot" ? undefined : data.serverId,
			organizationId: "",
		})
			.then(async () => {
								...
								render={({ field }) => (
									<FormItem>
										...
										<Select
											...
											defaultValue={
												field.value || (!isCloud ? "globalpivot" : undefined)
											}
										>
											<SelectTrigger>
												<SelectValue
													placeholder={!isCloud ? "GlobalPivot" : "Select a Server"}
												/>
											</SelectTrigger>
											<SelectContent>
												<SelectGroup>
													{!isCloud && (
														<SelectItem value="globalpivot">
